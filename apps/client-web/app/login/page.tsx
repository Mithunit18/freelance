  ...
  useEffect(() => {
    const checkSession = async () => {
      try {
        ...
        if (data && data.user) {
          const { role, onboarding_completed, client_onboarding_completed, email } = data.user;
          console.log('User role:', role, 'Onboarding completed:', onboarding_completed);

          // For clients, check client onboarding status
          if (role === "client") {
            const targetPath = client_onboarding_completed 
              ? `/client/dashboard/${encodeURIComponent(email)}`
              : "/client/onboarding";
    ...
    try {
      ...
      if (data && data.user) {
        ...
        if (data.user.role === "client") {
          ...
          const clientOnboardingCompleted = meData?.user?.client_onboarding_completed;
          const targetPath = clientOnboardingCompleted 
            ? `/client/dashboard/${encodeURIComponent(data.user.email)}`
            : "/client/onboarding";
