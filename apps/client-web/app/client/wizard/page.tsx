...
import type { Message, Creator, Stage, ChatError } from '@/types/chat';
...
const MessageBubble = ({ message }: { message: Message }) => {
  ...
  // Use creators from message object directly
  const creators = message.creators || [];
  
  // Clean up display content - remove JSON blocks for display
...
export default function WizardPage() {
    ...
    try {
      ...
      setTimeout(() => {
        ...
        setMessages([{
          id: '1',
          type: 'bot',
          content: response.greeting || response.message || "Hi! I'm here to help you find the perfect creator.",
          timestamp: new Date()
        }]);
      ...
      const errorMsg = err?.message || err?.code || 'Failed to start session. Please try again.';
      setError(errorMsg);
      setIsTransitioning(false);
      setIsLoading(false);
  ...
  const handleSendMessage = async () => {
    ...
    try {
      ...
      // Check session status for expiration or closure
      if (response.session_status === "closed" || response.session_status === "expired") {
      ...
      // Build bot message with proper error handling
      const messageContent = response.message || response.agent_response || 'Unable to process your request. Please try again.';
      ...
      // Handle creator recommendations
      if (response.has_results || (response.creators && response.creators.length > 0)) {
        // Extract creators from message if has_results flag is set
        let creatorList = response.creators || [];
        if (response.has_results && !creatorList.length && messageContent.includes('<<<CREATORS_JSON_START>>>')) {
          try {
            const jsonStart = messageContent.indexOf('<<<CREATORS_JSON_START>>>') + '<<<CREATORS_JSON_START>>>'.length;
            const jsonEnd = messageContent.indexOf('<<<CREATORS_JSON_END>>>');
            if (jsonEnd > jsonStart) {
              const jsonStr = messageContent.substring(jsonStart, jsonEnd).trim();
              creatorList = JSON.parse(jsonStr);
            }
          } catch (parseErr) {
            console.warn('Failed to parse creators from message:', parseErr);
          }
        }
        if (creatorList.length > 0) {
          localStorage.setItem('recommended_creators', JSON.stringify(creatorList));
          const redirectMsg: Message = {
            id: (Date.now() + 2).toString(),
            type: 'bot',
            content: 'Great! I found some matches for you. Redirecting to creator profiles...',
            timestamp: new Date()
          };
      ...
      } else if (messageContent.includes('```json')) {
        ...
        try {
          ...
          console.warn('Failed to parse creators from JSON block:', parseErr);
        }
      }
    } catch (err: any) {
      ...
      if (chatError.code === 'SESSION_EXPIRED') {
        errorMessage = 'Your session has expired. Please start a new conversation.';
        setSessionId(null);
      } else if (chatError.code === 'INVALID_INPUT') {
        errorMessage = chatError.message;
      } else if (chatError.code === 'SERVER_ERROR') {
        errorMessage = chatError.message;
      }
  ...
  const handleRestart = async () => {
    if (sessionId) {
      try {
        ...
        console.warn('Session deletion warning (non-critical):', err);
        // Session deletion is not critical, continue with restart
      }
    }
